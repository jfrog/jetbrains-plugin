...
**Invoking `jfrog-init` in Junie.** The skills are deployed **verbatim**, including `jfrog-init`'s `disable-model-invocation: true` flag. In the Junie IDE agent panel, run it by asking for it (type `/jfrog-init`, or *"run jfrog-init"*) — the model loads the skill on that explicit request. (Custom `/name` slash commands are a Junie **CLI** feature and aren't generated by this plugin, which targets the IDE.)
